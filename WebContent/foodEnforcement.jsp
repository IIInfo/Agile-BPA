...
<script>
	...
	$(document).ready(function(){
		...
		var reasonForRecallValue = document.getElementById("reasonForRecallValue").value;
		var classificationValue = document.getElementById("classificationValue").value;
		
		if((fromDateValue !='' && toDateValue!= '' && fromDateValue !="null" &&toDateValue!= "null" && fromDateValue != null && toDateValue != null)
			|| (reasonForRecallValue!='' && reasonForRecallValue!=null && reasonForRecallValue!="null") 
			|| (classificationValue!='' && classificationValue!=null && classificationValue!="null")){
			searchResultsByDate();	
	...
	function searchResultsByDate(){
		...
		if(fromDate !='' && toDate!='' && fromDate !="null" && toDate!= "null" && fromDate != null && toDate != null){
			var fromDateParts = fromDate.slice(0,10).split('/');
   		 	fromDateParameter = fromDateParts[2] +  fromDateParts[0] + fromDateParts[1];
		...
		if(reasonForRecall!='' && reasonForRecall!=null && reasonForRecall!="null"){
			 searchWithReasonForRecall = '+reason_for_recall:"'+reasonForRecall+'"';
			 document.getElementById("reasonForRecall").value = document.getElementById("reasonForRecallValue").value;
		}
		if(classification!=null && classification!="null"){
			 searchWithClassification = '+classification:"'+classification+'"';
			 document.getElementById("classification").value = document.getElementById("classificationValue").value;
		}
		//var openFdaSearchIndications='&search=openfda.indications_and_usage:{search}';
		
		var fdaUrl=openFdaEnforcementSite + openFdaApiKey + openFdaLimit + openFdaSearchFoodEnforcements + searchWithReasonForRecall + searchWithClassification;
		//alert(fdaUrl);
	...
	function retrieveResults() {
		...
		var openFdaSearchFoodEnforcements='&search=report_date:['+formattedFromDate+'+TO+'+formattedToDate+']';
	...
	function processResult(data) {
			...
			for( var ndx = 0; ndx < metaTotal; ndx++) {
				...
				var prodDetailLink = "&nbsp;&nbsp;&nbsp;<a href=\"FoodEnforcementServlet?recallNumber="+recallNumber+"&comingFrom="+comingFrom+"\"><i class=\"fa fa-lg fa-external-link\"></i>&nbsp;Details</a>";
            ...
            <div class="row">
             ...
             <div class="row =">
                <div class="col-xs-12">
                    ...
                    <p>Search and browse the latest food recalls. When an FDA-regulated product is either defective or potentially harmful, recalling that product (removing it from the market or correcting the problem) is the most effective means for protecting the public.</p>
                </div>
            </div><!--END ROW-->
             ...
             <form id ="formsubmit" action="FoodEnforcementServlet" method="get">
        		 ...
        		 <input type="hidden" name="reasonForRecallValue" id="reasonForRecallValue" value="<%= request.getParameter("reasonForRecall") %>"/>
       			 <input type="hidden" name="classificationValue" id="classificationValue" value="<%= request.getParameter("classification") %>"/>
        

                <div class="col-md-4 col-xs-12">
                    <div class="form-group">
                        <label for="keywords">Filter Results by Keyword(s):</label>
                        <input type="search" id="reasonForRecall" name="reasonForRecall" placeholder="&lt;chicken&gt; or &lt;chicken Maryland&gt;" class="form-control">
                    </div>
                </div>
                ...
                <div class="col-md-2 col-xs-6">
                    <div class="form-group">
                        ...
                        <div class="input-group" id="fromDatePicker">
                            <input type="text" aria-describedby="demo1HelpText2" maxlength="10" class="form-control hasDatepicker" id ="fromDate" name="fromDate"><span class="input-group-btn"><button class="btn btn-default ui-datepicker-trigger" type="button"><span><span class="fa fa-calendar" style="font-family :'FontAwesome' !important"><span class="sr-only">Calendar icon</span></span><span class="sr-only">Select date</span></span></button></span>
                        </div><!-- /input-group -->
                    </div>
                </div>
                ...
                <div class="col-md-2 col-xs-6">
                    <div class="form-group">
                        ...
                        <div class="input-group" id="toDatePicker">
                            <input type="text" aria-describedby="demo1HelpText2" maxlength="10" class="form-control hasDatepicker" id ="toDate" name="toDate"><span class="input-group-btn" id="toDatePickerGlyph"><button class="btn btn-default ui-datepicker-trigger" type="button"><span><span class="fa fa-calendar" style="font-family :'FontAwesome' !important"><span class="sr-only">Calendar icon</span></span><span class="sr-only">Select date</span></span></button></span>
                        </div><!-- /input-group -->
                    </div>
                </div>
                ...
                <div class="col-md-2 col-xs-6">
                    <div class="form-group">
                        <label for="select">Classification</label>
   			...
   			<div class="datesDisplayed">
                <div class="row">
                    <div class="col-xs-8 col-md-10">
				        ...
				        if((dateFrom == null && dateTo == null)||(dateFrom == "" && dateTo == "")||(dateFrom == "null" && dateTo == "null")){
					       
				        %>
				        	Results displayed are from <strong><%=fromDate%></strong> to <strong><%=toDate%></strong>
				        <%}else{ %>
				        	Results displayed are from <strong><%= request.getParameter("fromDate") %></strong> to <strong><%= request.getParameter("toDate") %></strong>
				        <%} %>
                    </div>
                    <div class="col-xs-4 col-md-2">
                        <a id="downloadCsv"><i class="fa fa-lg fa-download"></i> Download CSV</a>
                    </div>
